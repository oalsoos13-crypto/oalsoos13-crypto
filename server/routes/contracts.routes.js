...
function hydrate(c) {
  const items = db.prepare('SELECT * FROM contract_items WHERE contract_id=? ORDER BY sort,id').all(c.id)
    .map((it) => ({
      ...
      count: it.count, dimensions: it.dimensions, category: it.category, location: it.location, note: it.note,
    }));
  ...
  return {
    ...
    value: c.value, valueKind: c.value_kind, graceDays: c.grace_days, payWithin: c.pay_within,
...
const VALUE_KINDS = ['rent', 'support', 'marketing', 'other'];
...
router.post('/contracts', requireRole(...MGMT), asyncH((req, res) => {
  ...
  const hdr = {
    ...
    value, value_kind: VALUE_KINDS.includes(clean(b.valueKind)) ? clean(b.valueKind) : 'rent',
  ...
  const tx = db.transaction(() => {
    ...
    if (id) {
      db.prepare(`UPDATE contract_hdr SET code=@code,title=@title,subject_year=@subject_year,is_renewal=@is_renewal,
        ...
        period_from=@period_from,period_to=@period_to,renewable=@renewable,value=@value,value_kind=@value_kind,
      ...
      const r = db.prepare(`INSERT INTO contract_hdr (code,title,subject_year,is_renewal,party_rep,contract_date,
        level,coop,cust_id,period_from,period_to,renewable,value,value_kind,grace_days,pay_within,kind,parent_id,note,status,
        created_by,created_at,updated_by,updated_at)
        VALUES (@code,@title,@subject_year,@is_renewal,@party_rep,@contract_date,@level,@coop,@cust_id,@period_from,@period_to,
        @renewable,@value,@value_kind,@grace_days,@pay_within,@kind,@parent_id,@note,@status,@by,@now,@by,@now)`)
        .run({ ...hdr, by: req.user.id, now });
      id = r.lastInsertRowid;
    }
    const insIt = db.prepare(`INSERT INTO contract_items (contract_id,scope,cust_id,space_id,space,count,dimensions,category,location,note,sort)
      VALUES (?,?,?,?,?,?,?,?,?,?,?)`);
    items.forEach((it, i) => {
      ...
      insIt.run(id, sc, sc === 'outlet' ? clean(it.custId) : '', num(it.spaceId, null) || null, clean(it.space),
        num(it.count, 1), clean(it.dimensions), clean(it.category), clean(it.location), clean(it.note), i);
    });
...
router.get('/contracts/:id/debit-note', asyncH((req, res) => {
  ...
  const amount = Math.round((c.value * (billedMonths / contractMonths)) * 1000) / 1000;
  res.json({
